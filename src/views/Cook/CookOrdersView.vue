...
const LoadOrders = async ()=>{
  ...
  const responce = await apiFetch("get","/order/taken")
  if(responce.data){
...
const toggleEditMode = (id)=>{
  console.log(EditStatusData.value)
  let order = MyOrders.value.data.find(item=>item.id==(EditStatusData.value.id?EditStatusData.value.id:id))
  if(EditMode.value.Enabled){
    ...
    EditStatusData.value.newStatus = ''
      ...
      EditStatusData.value.oldStatus = order.status
...
const SwapStatus = (orderID)=>{
  ...
  if(currentOrder.status=="Готовится"){
    EditStatusData.value.newStatus = 'Готов'
  }
  else if(currentOrder.status=="Принят"){
    EditStatusData.value.newStatus = 'Готовится'
  }
  else{return}
...
const save = async ()=>{
  if (EditStatusData.value.oldStatus.length>1){
    let newStatus = ''
    if(EditStatusData.value.newStatus=="Готовится"){
      newStatus = "preparing"
    }
    else if(EditStatusData.value.newStatus=="Готов"){
      newStatus = "ready"
    }


    let body ={status:newStatus }
    if(EditStatusData.value.oldStatus=="Принят" && EditStatusData.value.newStatus=="Готов"){
      console.log(EditStatusData.value)
      const responce = await apiFetch("PATCH",`/order/${EditStatusData.value.id}/change-status`,{status:'preparing'})
          ...
          <li :id="order.id" class="overflow-y-auto transition items-center hover:scale-110 shadow-lg border col-span-12 sm:col-span-6 xl:col-span-4 rounded-xl flex flex-col gap-3 justify-between p-5" v-for="order of MyOrders.data">
            <div :class="{'justify-center':!orderFullScreen.state,'h-full':orderFullScreen.state}" class="w-fit md:w-full flex flex-col items-center">
                ...
                <div class="p-1 flex gap-1 items-center items-center flex-col">
                  ...
                  <div class="flex justify-between gap-2">
                    <span @click.prevent="SwapStatus(order.id)"
                          :class="{'hover:scale-110':EditMode.Enabled && (order.status=='Принят' || order.status=='Готовится'),'animate-pulse':EditMode.Enabled && (order.status=='Принят' || order.status=='Готовится'),'cursor-pointer':EditMode.Enabled && (order.status=='Принят' || order.status=='Готовится') ,'bg-red-500 ':order.status=='Отменен','bg-sky-500':order.status=='Принят','bg-yellow-500':order.status=='Готовится','bg-green-600 ':order.status=='Готов','bg-green-400 ':order.status=='Оплачен',}"
              ...
              <div class="w-full flex justify-center flex-col lg:flex-row items-center gap-1">
                ...
                <button v-if="orderFullScreen.state && (order.status=='Принят' || order.status=='Готов' || order.status=='Готовится' || EditMode.Enabled)" :class="{'scale-110':EditMode.Enabled}" @click.prevent="toggleEditMode(order.id)" class="z-1 justify-center w-fit transition ease-out
