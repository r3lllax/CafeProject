...
<template>
  <main class="w-full flex justify-center">
    <div class="w-full md:w-1/2 self-center justify-center p-5 flex gap-3 flex-col">
      ...
      <ul class="grid grid-cols-12 gap-3" v-if="MyOrders.data.length">
        <li class="transition items-center hover:scale-110 shadow-lg border col-span-12 sm:col-span-6 xl:col-span-4 min-h-28 min-w-28 rounded-xl flex flex-col gap-3 justify-between p-5" v-for="order of MyOrders.data">
          <div class="w-fit md:w-full flex flex-col items-center justify-center">
            ...
            <div class="w-full flex justify-center items-center">
              <button class="z-1 transition ease-out overflow-hidden relative hover:scale-110 border w-fit px-3 py-2 rounded-2xl flex items-center text-white before:transition before:ease-out before:duration-700 before:absolute before:border before:bg-sky-800 before:rounded-full before:min-w-10 before:min-h-10 before:text-white before:translate-x-[6rem] before:translate-y-[2.6rem] hover:before:scale-[10] hover:text-white hover:shadow-lg before:-z-10 font-semibold bg-sky-500">
                Подробнее
              </button>
            </div>
          </div>
